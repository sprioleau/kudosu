export default function getBoardRegionByPlace(place: number): number {
  // prettier-ignore
  const regions = {
  ...
  const regionKey = Object.entries(regions).find(([_, options]) => {
    return options.includes(place);
  });

  const region = regionKey ? Number(regionKey[0]) : 0;
